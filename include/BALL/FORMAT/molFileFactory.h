...
class BALL_EXPORT MolFileFactory
{
    public:
        ...
        static GenericMolFile* open(const String& name, File::OpenMode open_mode = std::ios::in);
				...
				/** Tries to opens a file with a desired default-format. 
				...
				*/
				...
				/** Returns a comma-separated string containing the file-extensions that are supported by MolFileFactory 
				 */
				static String getSupportedFormats();

				/** Returns true if the extension of the specified filename is supported; otherwise return false. 
				 */
